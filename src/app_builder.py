...
class AppBuilder:
    ...
    def watch(self):
        ...
        relative_template_directory = "{0}{1}{2}".format(AppBuilder.TEMPLATE_DIRECTORY, os.path.sep, AppBuilder.JABAL_BACKEND)
        io.directory.copy_directory_tree(relative_template_directory, output_directory, AppBuilder.OUTPUT_DIRECTORY)

        source_relative_directory = "{0}{1}".format(os.path.realpath(watch_path), os.path.sep)
        output_relative_directory = "{0}{1}".format(os.path.realpath(output_directory), os.path.sep)

        class_regex = re.compile(AppBuilder.CLASS_REGEX, re.IGNORECASE) # compiled so it's faster
        # when we embed, we copy all files, but delete non-modules from bin. Don't keep doing that
        # every second; remember and stop trying to copy these over.
        module_files_to_ignore = set({})
        ...
        while True:
            ...
            template_files = io.directory.traverse_for_mtime(relative_template_directory, {}, None, "{0}{1}".format(os.path.realpath(relative_template_directory), os.path.sep))
            ...
            # Don't treat module_files_to_ignore as new files
            added_files = {f for f in added_files if not f.full_path in module_files_to_ignore}

            with open("{0}{1}{2}{1}{3}".format(AppBuilder.TEMPLATE_DIRECTORY, os.path.sep, AppBuilder.JABAL_BACKEND, AppBuilder.MAIN_HTML_FILE)) as template_file:
            ...
            if len(added_files) > 0:
                ...
                for file in added_files:
                    if file.full_path in module_files_to_ignore:
                        continue
            ...
            if rebuild == True:
                if AppBuilder.VALID_COMMAND_LINE_ARGUMENTS["embed code"] in cmdline_arguments:
                    ...
                    for file in added_files:
                        ...
                        if file.filename.endswith('.py'):
                            with open(file.full_path) as python_file:
                                contents = python_file.read()
                            if class_regex.match(contents):
                                # It's not a module. Nuke it!
                                destination = "{0}{1}{2}".format(output_directory, os.path.sep, file.relative_path(source_relative_directory))
                                os.remove(destination)
                                print("Deleted {0} because it was copied over but isn't a module".format(destination))
                                module_files_to_ignore.add(file.full_path)

                    with open(main_file) as source_file:
                        main_code = source_file.read()

                    jabal_module_file = "{0}{1}{2}".format(relative_template_directory, AppBuilder.JABAL_MAIN_PY, os.path.sep)
                ...
                out_file_name = "{0}{1}{2}".format(output_directory, os.path.sep, AppBuilder.MAIN_HTML_FILE)
                with open(out_file_name, 'w+') as out_file:
                    substituted = original.replace(AppBuilder.CONTENT_PLACEHOLDER, main_code)
                    out_file.write(substituted)

                print("Rebuilt {0} at {1}".format(out_file_name, datetime.datetime.now()))
            time.sleep(0.5)
    ...
    def validate_args(self, args):
        ...
        main_file = "{0}{1}{2}".format(args[0], os.path.sep, AppBuilder.DEFAULT_MAIN_FILE)
    ...
    def inline_imports(self, watch_path, python_code):
        ...
        for match in matches:
            ...
            path_name = "{0}{1}{2}".format(watch_path, module_name.replace('.', os.path.sep), os.path.sep)
            path_name += ".py"
